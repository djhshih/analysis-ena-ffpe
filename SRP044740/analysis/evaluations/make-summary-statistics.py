...
lookup_table = pl.read_csv("../../annot/sample-info_stage1.tsv", separator="\t")

## Make a dataframe with only the FFPE samples
ffpe_vcf_ids = lookup_table.filter(pl.col("sample_type") == "FFPE").get_column("sample_alias")
...
for path in ffpe_vcf_paths:
	...
	sample_alias = sample_id.split("_")[0]
	run_acc = sample_id.split("_")[-1]
	sample_number = lookup_table.filter((pl.col("run_accession") == run_acc)).get_column("sample_number")[0]
	...
	truth_annot = (
     	lookup_table
    	.filter(
         	(pl.col("sample_number") == sample_number) &
			(pl.col("sample_type") == "FROZ")
        ))
