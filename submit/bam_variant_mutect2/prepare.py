...
base = {
    ...
    # 'bam_variant_mutect2.intervals': os.path.join(intervals_path, 'CPOS_812_HPV_EBV_targets_hg38_sorted_merged_mod_filtered.interval_list'),
    ...
    "bam_variant_mutect2.gnomad_idx": os.path.join(
        vcf_path, "af-only-gnomad.hg38.vcf.gz.tbi"
    ),
    "bam_variant_mutect2.variants_for_contamination": os.path.join(
        vcf_path, "small_exac_common_3.hg38.vcf.gz"
    ),
    "bam_variant_mutect2.variants_for_contamination_idx": os.path.join(
        vcf_path, "small_exac_common_3.hg38.vcf.gz.tbi"
    ),
    "bam_variant_mutect2.realignment_index_bundle": os.path.join(
        bundle_path, "Homo_sapiens_assembly38.index_bundle"
    ),
    "bam_variant_mutect2.m2_extra_args": "--disable-read-filter NotDuplicateReadFilter --downsampling-stride 50 --linked-de-bruijn-graph",  # --dont-use-soft-clipped-bases --max-reads-per-alignment-start 500
