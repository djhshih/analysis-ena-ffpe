...
def eval_mut_sig(vcf_path, reference_genome, outdir_root) -> None:
	...
	variants_96c = ms.variants_mut_profile(vcf_path, f"{sample_name}_mobsnvf", reference_genome)
	mutaion_counts_96c = ms.create_96c_mutation_counts(variants_96c)

	print(f"{sample_name}:")

	variants_96c.write_csv(f"{outdir}/{sample_name}_mobsnvf_96c_mutation_profiles.tsv", separator="\t")
	...
	print(f"\tSaved 96 channnel mutation counts to: {outdir}/{sample_name}_mobsnvf_96c_mutation_count.tsv")

	ms.SBS96_plot(mutaion_counts_96c[:, 2].to_numpy(), label=f"{sample_name}_mobsnvf", height=3, width=10, s=8, xticks_label=False, file=f"{outdir}/{sample_name}_mobsnvf_96c_mutation_signature.pdf")
