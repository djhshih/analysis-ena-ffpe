...
def import_formatted_vcf(vcf_path: str) -> pl.DataFrame:
	"""
...
def make_mut_info(vcf_path: str, sample_name: str, genome: pysam.FastaFile) -> pl.DataFrame:
    
	mut_info = import_formatted_vcf(vcf_path).with_columns(pl.lit(sample_name).alias("Sample"))
...
for i in range(lookup_table_ffpe.shape[0]):
	...
	mut_info = make_mut_info(lookup_table_ffpe[i, "vcf_path"], sample_name, genome)
...
for i in range(lookup_table_ffpe.shape[0]):
	...
	mut_info = make_mut_info(lookup_table_ffpe, i, sample_name, genome)
